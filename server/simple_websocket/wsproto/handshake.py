...
from .h11 import Connection as h11_Connection
from .h11 import SERVER as h11_SERVER
from .h11 import CLIENT as h11_CLIENT
from .h11 import Request as h11_Request
from .h11 import RemoteProtocolError as h11_RemoteProtocolError
from .h11 import NEED_DATA as h11_NEED_DATA
from .h11 import PAUSED as h11_PAUSED
from .h11 import ConnectionClosed as h11_ConnectionClosed
from .h11 import InformationalResponse as h11_InformationalResponse
from .h11 import Response as h11_Response
from .h11 import Data as h11_Data
from .h11 import EndOfMessage as h11_EndOfMessage
...
class H11Handshake:
    ...
    def __init__(self, connection_type: ConnectionType) -> None:
        ...
        if self.client:
            self._h11_connection = h11_Connection(h11_CLIENT)
        else:
            self._h11_connection = h11_Connection(h11_SERVER)
    ...
    def initiate_upgrade_connection(
        ...
        upgrade_request = h11_Request(method=b"GET", target=path, headers=headers)
        h11_client = h11_Connection(h11_CLIENT)
    ...
    def receive_data(self, data: Optional[bytes]) -> None:
        ...
        while True:
            ...
            except h11_RemoteProtocolError:
                raise RemoteProtocolError(
                    "Bad HTTP message", event_hint=RejectConnection()
                )
            if (
                isinstance(event, h11_ConnectionClosed)
                or event is h11_NEED_DATA
                or event is h11_PAUSED
            ):
                break

            if self.client:
                if isinstance(event, h11_InformationalResponse):
                ...
                elif isinstance(event, h11_Response):
    ...
    def _accept(self, event: AcceptConnection) -> bytes:
        ...
        response = h11_InformationalResponse(
    ...
    def _reject(self, event: RejectConnection) -> bytes:
        ...
        response = h11_Response(status_code=event.status_code, headers=headers)
        data = self._h11_connection.send(response) or b""
        self._state = ConnectionState.REJECTING
        if not event.has_body:
            data += self._h11_connection.send(h11_EndOfMessage()) or b""
            self._state = ConnectionState.CLOSED
        return data
    ...
    def _send_reject_data(self, event: RejectData) -> bytes:
        ...
        data = self._h11_connection.send(h11_Data(data=event.data)) or b""
    ...
    def _establish_client_connection(
        self, event: h11_InformationalResponse
