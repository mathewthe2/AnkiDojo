...
#     1: ['LH', 'HL'],
#     2: ['LHH', 'HLL', 'LHL'],
#     3: ['LHHH', 'HLLL', 'LHLL', 'LHHL'],
#     4: ['LHHHH', 'HLLLL', 'LHLLL', 'LHHLL', 'LHHHL'],
#     5: ['LHHHHH', 'HLLLLL', 'LHLLL']
# }

def pitch_value_to_patt(word, pitch_value):
    number_of_mora = len(hira_to_mora(word))
    if number_of_mora >= 1:
        if pitch_value == 0:
            # heiban
            return 'L' + 'H'*number_of_mora
        elif pitch_value == 1: 
            # atamadaka
            return 'H' + 'L'*number_of_mora
        elif pitch_value >= 2:
            # nakadaka / odaka
            stepdown = pitch_value - 2
            return 'LH' + 'H'*stepdown + 'L'*(number_of_mora - pitch_value + 1)
    return ''
